...
static const size_t MAX_ACTION_STR_LEN = 10;
...
static const char *action_str(Action action) {
  switch (action) {
  case Action::Connect:
    return "connect";
  case Action::Chat:
...
static Action action_from_string(std::string_view sv) {
  if (sv == "connect") {
...
static void from_json(const nlohmann::json &, Message &);
static void to_json(nlohmann::json &, const Message &);
...
struct Message {
  ...
  void deserialize(std::string_view sv) {
    try {
      auto msg_json = nlohmann::json::parse(sv);
...
void to_json(nlohmann::json &j, const Message &msg) {
  j = nlohmann::json{{"sender", msg.sender},
